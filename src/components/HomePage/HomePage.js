          ...
          <div className={classes.column}>
            ...
            <div className={classes.column}>
              ...
              <img
                ...
                alt=""
              />
              Now, you must set what the reference width for the image is. If
              it's automatic (default) it will measure the length of the green
              line (c) and set that to be equal to the referense width (a). If
              you set it to manual (b), it will instead take the width of the
              image, and set that to the reference width (a). This way the
              software knows how big the wound is in real life, and not just in
              pixels.
              <img
                src={stepChooseWidth}
                className={classes.tutorialImage}
              ...
              recommended to first crop the image (1) to a smaller size that
              still includes the wounds and the green line (for auto-width), so
              that if you need to manually adjust the mask it is easier to see
              the borders. Second, we now select the mask (2) to identify what
              type of wound we're looking for, specifically this changes the HSV
              color ranges for finding what counts as the wound. Finally, press
              the MEASURE AREA button (3) and the areas will be calculated.
              <img src={stepMeasure} className={classes.tutorialImage} alt="" />
              After pressing the button, the image will be sent to a server
              which analyses the image. After a few seconds, the server should
              finish analysis and send it back to your computer- after which
              there will be a popup as shown below. Press ok on the popup.
              <img src={stepPressOk} className={classes.tutorialImage} alt="" />
              Borders are now generated on the image, and the areas of each
              border (from left to right) are calculated and displayed.
              <img
                src={stepImageMasked}
                className={classes.tutorialImage}
                alt=""
              ...
              two ways to make the border closer to the wound. The first
              (easier) is to look at the grid below. The grid automatically
              adjusts the wound mask to stricter and looser settings and
              displays. The center image is the image with the original mask. By
              clicking on any of the images, you will be able to re-analyze the
              ...
              <img
                ...
                alt=""
              />
              After clicking, and then pressing the ok button like before, you
              can now see that a new grid of stricter and looser masks is
              generated around the image selected in the last step. Similarly as
              we clicked the top left image before, the original settings are
              now in the bottom right image.
              <img
                src={stepGridMasks2}
                className={classes.tutorialImage}
                alt=""
              ...
              below, corresponding to the wounds on the image from left to
              right.
              <img src={stepAreas} className={classes.tutorialImage} alt="" />
              ...
              <img
                ...
                alt=""
              />
              Once you press ok from the popup, you will then see all the
              images. They are listed in order, using the green line as a width
              reference. The areas are listed for the wounds in order of
              left-to-right just like in single analysis. For most images, the
              mask will not be exactly correct, so you can click on the image
              itself in order to automatically go to the single-image
              measurement page with that image automatically transfered.
              <img
                src={stepMultiSelect}
                className={classes.tutorialImage}
                alt=""
              ...
              <li>
                ...
                I've been unable to solve it, since it's not critical, I've put
                it on the backlog while I work on other more critical tasks.
              </li>
              <li>
                Since this web application is hosted for free on Heroku, after a
                long period of inactivity the frontend app and backend server
                will shutdown, so if you load the page after a long time it will
                likely give you an error at first. Don't worry, this is just
                because the server is stopped. After you tried to open the page
                (url request), it asked Heroku to restart the server and by the
                time it gives you the error, it has already started up again and
                you can just reload the page and you'll see the app.
              </li>
            </div>
          </div>
        </div>
